...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            // Checking the height to see if Genesis Block
            let chainHeight =  await self.getChainHeight();

            if (chainHeight >=0) { // Not the Genesis Block
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // Get Time from the message parameter
            ...
            await self.chain.forEach((block, idx) => {
              ...
              if (block.height > 0) {
                if (block.previousBlockHash !== self.chain[idx - 1].hash) {
                  errorLog.push({"blockHeight": block.height, "validPreviousHash": false});
                }
              }
            });
            resolve(errorLog);
